...
return [
    ...
    'DB.connection'=> 'mysql:dbname=chat-room;host=localhost',
    'DB.username'=> 'chat-room',
    'DB.password'=> 'xzweHpKNC6asNCyX',
    'DB.options' => [],

    //自己定义配置
    'my.jwtKey' => 'greawtyreyw',
    'my.environment' => 'develop', //production
    ...
    ************************************************************************************/
    \Doctrine\Common\Cache\Cache::class => \DI\object(\Doctrine\Common\Cache\FilesystemCache::class)
        ->constructorParameter('directory', $_SERVER['DOCUMENT_ROOT'] . '/../cache/'), //sys_get_temp_dir() // 指定系统缓存文件目录
    ...
    $this->logger->info('write a log');*/
    'defaultLoggerStream' => \DI\object(\Monolog\Handler\StreamHandler::class)
        ->constructor($_SERVER['DOCUMENT_ROOT'] . '/logs/' . date("Y-m-d") . '.txt', \Monolog\Logger::WARNING),
    ...
    \PhpBoot\Controller\ExceptionRenderer::class =>
        \DI\object(\App\Utils\ExceptionRender::class),
